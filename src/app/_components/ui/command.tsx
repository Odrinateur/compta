...
function Command({
    ...
    return (
        <CommandPrimitive
            data-slot="command"
            className={cn(
                "bg-popover rounded-md flex flex-col w-full h-full overflow-hidden text-popover-foreground",
                className
            )}
            {...props}
        />
...
function CommandInput({
    ...
    return (
        <div
            ...
            className="border-b flex items-center gap-2 px-3 h-9"
        >
            <SearchIcon className="opacity-50 size-4 shrink-0" />
            <CommandPrimitive.Input
                data-slot="command-input"
                className={cn(
                    "bg-transparent disabled:opacity-50 rounded-md outline-hidden flex py-3 disabled:cursor-not-allowed w-full h-10 placeholder:text-muted-foreground text-sm",
                    className
                )}
                {...props}
            />
